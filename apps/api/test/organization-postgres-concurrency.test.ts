...
integration("Organization PostgreSQL concurrency", () => {
  beforeAll(async () => {
    ...
    try {
      ...
      await setup.query(`
        ...
        CREATE TABLE organization_incumbencies (id uuid PRIMARY KEY, change_set_id uuid NOT NULL, position_key uuid NOT NULL, employee_id uuid NULL, account_id uuid NULL, kind text NOT NULL, is_primary_structural boolean NOT NULL DEFAULT false, effective_from date NOT NULL, effective_to date NULL, reason text NULL);
  ...
  it("makes the second overlapping mutation wait and then read the first committed snapshot", async () => {
    ...
    let snapshotBPromise: Promise<Awaited<ReturnType<PostgresOrganizationRepository["loadEditableSnapshotForUpdate"]>>> | undefined;
    try {
      ...
      snapshotBPromise = secondRepository.loadEditableSnapshotForUpdate(changeSetId)
