...
function createPool(input: {
  ...
  const rootQuery = vi.fn(async (sql: string) => {
    ...
    throw new Error(`Unexpected root SQL in roster query route test: ${sql}`);
  });
  ...
  const clientQuery = vi.fn(async (sql: string, params?: unknown[]) => {
    ...
    if (sql.includes("SELECT EXISTS") && sql.includes("attendance_adms_device_roster_entries")) {
      expect(params).toEqual([deviceId]);
      return { rows: [{ observed: safeRosterObserved }], rowCount: 1 };
    }
    if (sql.includes("FROM attendance_adms_commands") && sql.includes("status IN")) {
      return input.activeCommand
        ? { rows: [{ id: "00000000-0000-4000-8000-000000000912", commandNumber: "20", status: "delivered" }], rowCount: 1 }
        : { rows: [], rowCount: 0 };
    }
    if (sql.includes("INSERT INTO attendance_adms_commands")) {
      expect(params?.[1]).toBe(deviceId);
      expect(params?.[2]).toBe("DATA QUERY USERINFO");
      return {
        rows: [
          {
            id: params?.[0],
            ...
            expiresAt: new Date("2026-08-30T16:15:00.000Z"),
          },
        ],
        rowCount: 1,
      };
    ...
    throw new Error(`Unexpected transaction SQL in roster query route test: ${sql}`);
  });
...
describe("ATT-005 Wave 2 full roster USERINFO canary route", () => {
  it("queues one exact audited observed-only roster query for an active device", async () => {
    ...
    const response = await app.inject({
      method: "POST",
      url: `/admin/attendance/adms/devices/${deviceId}/commands/query-user-info-roster`,
  ...
  it("rejects employee principals before opening a command transaction", async () => {
  ...
  it("requires an existing safe USERINFO roster observation", async () => {
    const { pool, clientQuery } = createPool({ safeRosterObserved: false });
    const app = Fastify({ logger: false });
    await registerAdmsWave2UserInfoRoutes(app, pool, config);
    ...
    expect(response.json()).toMatchObject({ code: "ADMS_SAFE_ROSTER_NOT_OBSERVED" });
  ...
  it("does not queue while another command is active", async () => {
    ...
    expect(response.statusCode).toBe(409);
    expect(response.json()).toMatchObject({ code: "ADMS_COMMAND_ACTIVE" });
    expect(clientQuery.mock.calls.some(([sql]) => String(sql).includes("INSERT INTO attendance_adms_commands"))).toBe(false);
    await app.close();
  });
});
